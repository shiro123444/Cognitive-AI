...
def test_edufish_templates_endpoint(client):
    response = client.get("/api/v1/edu/templates")
...
def test_edufish_normalize_endpoint_reconciles_chinese_headers(client):
    response = client.post("/api/v1/edu/datasets/normalize", json=sample_education_payload())
...
def test_edufish_analysis_preview_builds_report_and_graph(client):
    response = client.post("/api/v1/edu/analysis/preview", json={
...
def test_edufish_dataset_analysis_report_closed_loop(client):
    dataset_response = client.post("/api/v1/edu/datasets", json={
        **sample_education_payload(),
    ...
    list_response = client.get("/api/v1/edu/datasets")
    assert list_response.status_code == 200
    assert list_response.get_json()["data"]["count"] == 1

    run_response = client.post("/api/v1/edu/analysis/run", json={
    ...
    status_response = client.get(f"/api/v1/edu/analysis/status/{run_payload['job_id']}")
    ...
    analysis_response = client.get(f"/api/v1/edu/analysis/{run_payload['analysis_id']}")
    assert analysis_response.status_code == 200
    analysis = analysis_response.get_json()["data"]
    assert analysis["status"] == "completed"
    assert analysis["summary"]["quality_overview"]["avg_feedback_rating"] == 3.95

    graph_response = client.get(f"/api/v1/edu/analysis/{run_payload['analysis_id']}/graph")
    assert graph_response.status_code == 200
    graph = graph_response.get_json()["data"]
    assert len(graph["nodes"]) > 0
    assert len(graph["edges"]) > 0

    report_response = client.get(f"/api/v1/edu/reports/{run_payload['report_id']}")
    assert report_response.status_code == 200
    report = report_response.get_json()["data"]
    assert report["status"] == "completed"
    assert "Course Quality Signals" in report["markdown_content"]

    preview_response = client.get(f"/api/v1/edu/reports/{run_payload['report_id']}/preview")
    ...
    pdf_response = client.get(f"/api/v1/edu/reports/{run_payload['report_id']}/pdf")
...
def test_edufish_course_scope_analysis_and_prediction_closed_loop(client):
    dataset_response = client.post("/api/v1/edu/datasets", json=sample_education_payload())
    dataset = dataset_response.get_json()["data"]
    ...
    prediction_response = client.get(f"/api/v1/edu/analysis/{run_payload['analysis_id']}/prediction")
...
def test_edufish_latest_analysis_endpoint_returns_latest_completed_course_analysis(client):
    ...
    first_run = client.post("/api/v1/edu/analysis/run", json={
    ...
    second_run = client.post("/api/v1/edu/analysis/run", json={
        "dataset_id": dataset_id,
        "template_id": "course-quality",
        "audience_role": "school_admin",
        "scope": {
    ...
    latest_response = client.get("/api/v1/edu/analysis/latest?course_id=AI101")
